...
export default function Navbar({ transparent: propTransparent }: NavbarProps) {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (mobileMenuRef.current && !mobileMenuRef.current.contains(event.target as Node) && !(event.target as HTMLElement).closest('button[onClick*="setIsMobileMenuOpen"]')) {
        // Only close if we didn't click the toggle button itself (handled by its own onClick)
        // Actually mobile menu usually has an overlay, but let's stick to profile for now.
      }
    }
    ...
    {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-white z-[9999] xl:hidden flex flex-col animate-in slide-in-from-right duration-300">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-8 flex flex-col gap-10">
            {/* Dynamic Navigation Based on Role/Page */}
            <div className="flex flex-col gap-4">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-2 mb-2">
                {pathname?.startsWith('/admin') ? 'Admin Control' : pathname?.startsWith('/agent') ? 'Agent Dashboard' : 'Main Navigation'}
              </p>
              <nav className="flex flex-col">
                {(pathname?.startsWith('/admin') ? [
                  { label: '📊 Analytics', href: '/admin?section=overview' },
                  { label: '🏠 All Properties', href: '/admin?section=properties' },
                  { label: '👥 Users', href: '/admin?section=users' },
                  { label: '📝 Agent Apps', href: '/admin?section=agents' },
                  { label: '🎖️ Agents', href: '/admin?section=agents_list' },
                  { label: '📋 Interests', href: '/admin?section=interests' },
                  { label: '📩 Service Queries', href: '/admin?section=service_requests' },
                  { label: '💸 Transactions', href: '/admin?section=transactions' },
                  { label: '⚙️ Plan Settings', href: '/admin?section=plans' },
                  { label: '⭐ Reviews', href: '/admin?section=reviews' },
                  { label: '🏠 Site Home', href: '/' },
                ] : pathname?.startsWith('/agent') ? [
                  { label: '📊 Overview', href: '/agent?tab=overview' },
                  { label: '💸 Transactions', href: '/agent?tab=transactions' },
